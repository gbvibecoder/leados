...
const statusConfig: Record<AgentStatus, { icon: typeof Bot; color: string; bg: string; label: string }> = {
  idle: { icon: Clock, color: 'text-gray-400', bg: 'bg-white/5', label: 'Idle' },
  ...
  if (compact) {
    return (
      <div
        onClick={onClick}
        className={cn(
          'flex cursor-pointer items-center gap-3 rounded-lg border p-3 transition-all hover:border-cyan-500/20',
          isActive ? 'border-cyan-500 bg-cyan-950/20' : 'border-white/[0.04] bg-[rgba(2,2,5,0.6)]/50',
      ...
      >
        ...
        <span className="flex h-6 w-6 items-center justify-center rounded-full bg-white/5 text-xs text-gray-400">
          {order}
        </span>
      </div>
    );
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="flex h-5 w-5 items-center justify-center rounded-full bg-white/5 text-xs text-gray-400">
                {order}
              </span>
        ...
        {status === 'idle' && onRun && (
          <button
            ...
            className="rounded-lg p-2 text-gray-500 opacity-0 transition-opacity hover:bg-white/5 hover:text-white group-hover:opacity-100"
          >
      ...
      {outputPreview && status === 'done' && (
        <p className="mt-2 truncate text-xs text-gray-400">{outputPreview}</p>
      )}
